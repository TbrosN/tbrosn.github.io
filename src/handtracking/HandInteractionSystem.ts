...
export class HandInteractionSystem {
  ...
  async init(): Promise<void> {
    ...
    this.handTracker.onResults((results) => {
      const { indexTip, thumbTip, cursorPosition } =
        this.handMapper.update(results);
  ...
  private handleGesture(
    ...
    const justPinched =
      state === "pinched" && this.lastGestureState !== "pinched";
    const justReleased =
      state === "idle" && this.lastGestureState === "pinched";
